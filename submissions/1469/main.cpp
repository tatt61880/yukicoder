...
static void k_i();
static void k_j();
static void k_l(int64_t k_aw /*excpt*/);
static void k_p(type_(k_o) k_az /*me*/);
static void k_q(k_o* k_ba);
static int64_t k_r(type_(k_o) k_bb /*me*/, type_(k_o) k_bc /*t*/);
static type_(k_o) k_s(type_(k_o) k_bd /*me*/);
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bg /*me*/);
static type_(k_o) k_u(type_(k_o) k_bi /*me*/, type_(Array_<uint8_t>) k_bj /*bin*/, int64_t* k_bk /*idx*/);
static type_(Array_<char16_t>) k_v(type_(k_o) k_bn /*me*/);
static void k_w(type_(k_n) k_bo /*me*/);
using k_bq=void(*)(type_(k_o));
static int64_t k_x(type_(k_n) k_br /*me*/, int64_t k_bs /*min*/, int64_t k_bt /*max*/);
...
static void k_aa(type_(k_n) k_co /*me*/, uint32_t k_cp /*seed*/);
static uint32_t k_ab(type_(k_n) k_cv /*me*/);
static void k_ac(k_n* k_cx);
static type_(k_n) k_ad(type_(k_n) k_cy);
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_db);
static type_(k_n) k_af(type_(k_n) k_dh, type_(Array_<uint8_t>) k_di, int64_t* k_dj);
static type_(k_n) k_ag(type_(k_n) k_dm /*me2*/, uint32_t k_dn /*seed*/);
using k_do=void(*)(type_(k_n), uint32_t);
template<typename T_>static T_ k_ai(T_ k_dp /*me_*/, type_(Array_<int64_t>) k_dq /*type*/, T_ k_dr /*n*/);
static int64_t k_aj();
static int64_t k_ak();
static type_(Array_<char16_t>) k_an();
static void k_av(type_(Array_<char16_t>) k_em /*str*/);
template<typename T_>static T_ k_ch(T_ k_en /*me_*/, type_(Array_<int64_t>) k_eo /*type*/, T_ k_ep /*n*/);
template<typename T_>static T_ k_ci(T_ k_eq /*me_*/, type_(Array_<int64_t>) k_er /*type*/, int64_t k_es /*n*/);
template<typename T_>static T_ k_cu(T_ k_ev /*me_*/, type_(Array_<int64_t>) k_ew /*type*/, int64_t k_ex /*n*/);
static char16_t k_dy();
template<typename T_>static T_ k_el(T_ k_fa /*me_*/, type_(Array_<int64_t>) k_fb /*type*/, int64_t k_fc /*start*/, int64_t k_fd /*len*/);
...
k_n::k_n(): k_o(), k_cq(), k_cr(), k_cs(), k_ct(){ Y = 8; }
k_n::~k_n(){
k_ac(this);
}
// k_main_
...
int64_t k_as;
(k_am) = ((k_an)());
for(k_ar = (0LL), k_as = (((k_am)->Len()) - (1LL)); k_ar <= k_as; k_ar += (1LL)){
if(((k_ar) == (0LL)) || (((k_am)->At(k_ar)) != ((k_am)->At((k_ar) - (1LL))))){
(k_av)((new_(Array_<char16_t>)(((k_am)->At(k_ar)))));
}
}
(k_av)((newArrayBin_<char16_t>(1,"\x0A\x00")));
}
// _finVars
static void k_i(){
(k_al) = (nullptr);
...
type_(k_o) k_be;
type_(k_o) k_bf;
(k_be) = (k_bf = new_(k_o)(), k_p(k_bf), k_bf);
return as_<k_o>(classTable_, (k_be), 0);
}
// _toBin
static type_(Array_<uint8_t>) k_t(type_(k_o) k_bg /*me*/){
...
static uint32_t k_ab(type_(k_n) k_cv /*me*/){
uint32_t k_cw; // t
(k_cw) = (k_ai<uint32_t>(((k_cv)->k_cq), (nullptr), (k_ci<uint32_t>(((k_cv)->k_cq), (nullptr), (11LL)))));
((k_cv)->k_cq) = ((k_cv)->k_cr);
((k_cv)->k_cr) = ((k_cv)->k_cs);
((k_cv)->k_cs) = ((k_cv)->k_ct);
((k_cv)->k_ct) = (k_ai<uint32_t>((k_ai<uint32_t>(((k_cv)->k_ct), (nullptr), (k_cu<uint32_t>(((k_cv)->k_ct), (nullptr), (19LL))))), (nullptr), (k_ai<uint32_t>((k_cw), (nullptr), (k_cu<uint32_t>((k_cw), (nullptr), (8LL)))))));
return (k_cv)->k_ct;
}
// _dtor
static void k_ac(k_n* k_cx){
...
type_(k_n) k_cz;
type_(k_n) k_da;
(k_cz) = (k_da = new_(k_n)(), k_w(k_da), k_da);
((k_cz)->k_cq) = ((k_cy)->k_cq);
((k_cz)->k_cr) = ((k_cy)->k_cr);
((k_cz)->k_cs) = ((k_cy)->k_cs);
((k_cz)->k_ct) = ((k_cy)->k_ct);
return as_<k_n>(classTable_, (k_cz), 8);
}
// _toBin
static type_(Array_<uint8_t>) k_ae(type_(k_n) k_db){
...
// input
static type_(Array_<char16_t>) k_an(){
type_(Array_<char16_t>) k_ds; // buf
int64_t k_dt; // ptr
char16_t k_dx; // c
(k_ds) = (newArrays_<type_(Array_<char16_t>)>((1024LL)));
(k_dt) = (0LL);
while(true){
(k_dx) = ((k_dy)());
if((k_dx) == (u'\u000D')){
...
if((k_dt) == ((k_ds)->Len())){
(k_ds) = ((k_ds)->Cat(newArrays_<type_(Array_<char16_t>)>(((k_ds)->Len()))));
}
((k_ds)->At(k_dt)) = (k_dx);
(k_dt) = ((k_dt) + (1LL));
}
return k_el<type_(Array_<char16_t>)>((k_ds), (nullptr), (0LL), (k_dt));
}
// print
static void k_av(type_(Array_<char16_t>) k_em /*str*/){
if(k_em==nullptr){
